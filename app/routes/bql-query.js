...
export default Route.extend(Queryable, {
  ...
  actions: {
    fireQuery() {
      this.get('queryManager').addBQLResult(this.paramsFor('bql-query').query_id).then(result => {
        this.store.findRecord('bql-query', this.paramsFor('bql-query').query_id).then(query => {
  ...
  model(params) {
    return hash({
      ...
      query: this.store.findRecord('bql-query', params.query_id)
    }).catch(() => {
